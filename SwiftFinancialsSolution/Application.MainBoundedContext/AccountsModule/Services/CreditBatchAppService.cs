...
>>>>>>> Stashed changes
                                            ...
                                            else if (productTypeCode == "DEP")
                                            {
                                                ...
                                                {
                                                    Column1 = dataRecord[0],                         // Payroll Number
                                                    Column2 = total.ToString(),                      // Amount (for ParseCheckOff)
                                                    Column3 = total.ToString(),                      // Amount (for ParseCheckOff_Fuzzy)
                                                    Column4 = "1",                                   // Product Code / productBalance placeholder
                                                    Column5 = nameof(CheckOffEntryType.sShare),      // EntryType (for ParseCheckOff)
                                                    Column6 = dataRecord[4],                         // Reference
                                                    Column7 = nameof(CheckOffEntryType.sShare),      // EntryType (for ParseCheckOff_Fuzzy)
                                                };
                                                importEntries.Add(checkOffEntry);
                                            }
                                            ...
                                            else if (productTypeCode == "RBF")
                                            {
                                                var checkOffEntry = new BatchImportEntryWrapper
                                                {
                                                    ...
                                                    Column6 = dataRecord[4],
                                                    Column7 = nameof(CheckOffEntryType.sShare),
                                                };
                                                importEntries.Add(checkOffEntry);
                                            }
                                            ...
                                            else
                                            {
                                                ...
                                                // Post any remaining amount — ORIGINAL UNCHANGED
                                                if (total > 0)
                                                {
                                                    var depositEntry = new BatchImportEntryWrapper
                                                    {
                                                        ...
                                                        Column2 = total.ToString(),
                                                        Column3 = dataRecord[3],
                                                        Column4 = "222",  // ← investment account for excess, NOT deposits
                                                        Column5 = "1",    // ← ORIGINAL
                                                        Column6 = dataRecord[4],
                                                    };
                                                    importEntries.Add(depositEntry);
                    ...
                    if (standingOrderDTOs != null && standingOrderDTOs.Any())
                    {
                        ...
                        switch ((CheckOffEntryType)Enum.Parse(typeof(CheckOffEntryType), item.Column7))
                        {
                            ...
                            case CheckOffEntryType.wLoan:
                                ...
                                // First try investment standing orders (original logic)
                                var sShare_MatchedStandingOrders = standingOrderDTOs
                                    .Where(x => x.BeneficiaryCustomerAccountCustomerAccountTypeProductCode == (int)ProductCode.Investment
                                             && x.ChargeFixedAmount == contributionAmount);

                                if (sShare_MatchedStandingOrders != null && sShare_MatchedStandingOrders.Any())
                                {
                                    // ── original investment logic unchanged ──
                                    foreach (var targetInvestmentStandingOrder in sShare_MatchedStandingOrders)
                                    {
                                        var targetCustomerAccount = _sqlCommandAppService
                                            .FindCustomerAccountById(targetInvestmentStandingOrder.BeneficiaryCustomerAccountId, serviceHeader);
                                ...
                                {
                                    // ── NEW: Try savings standing orders (DEP/RBF/SHP) ──
                                    var sShare_MatchedSavingsStandingOrders = standingOrderDTOs
                                        .Where(x => x.BeneficiaryCustomerAccountCustomerAccountTypeProductCode == (int)ProductCode.Savings
                                                 && x.ChargeFixedAmount == contributionAmount);

                                    if (sShare_MatchedSavingsStandingOrders != null && sShare_MatchedSavingsStandingOrders.Any())
                                    {
                                        foreach (var targetSavingsStandingOrder in sShare_MatchedSavingsStandingOrders)
                                        {
                                            var targetCustomerAccount = _sqlCommandAppService
                                                .FindCustomerAccountById(targetSavingsStandingOrder.BeneficiaryCustomerAccountId, serviceHeader);
                                            ...
                                            creditBatchEntry.CustomerAccountCustomerIndividualPayrollNumbers = item.Column2;
                                            creditBatchEntry.Beneficiary = item.Column5;
                                            creditBatchEntry.ProductDescription = targetSavingsStandingOrder.BeneficiaryProductDescription;
                                            creditBatchEntry.Principal = contributionAmount;
                                            creditBatchEntry.Balance = productBalance;
                                            creditBatchEntry.Reference = item.Column2;
                                            result.MatchedCollection1.Add(creditBatchEntry);
                                    ...
                                    {
                                        item.Remarks = string.Format(
                                            "Record #{0} ~ (sShare/wCont/sInvest/sRisk/wLoan) no match for standing order having ChargeFixedAmount as {1}",
