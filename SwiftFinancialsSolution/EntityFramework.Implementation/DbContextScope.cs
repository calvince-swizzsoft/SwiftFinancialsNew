...
namespace Numero3.EntityFramework.Implementation
{
    public class DbContextScope : IDbContextScope
    {
        ...
        public int SaveChanges(ServiceHeader serviceHeader)
        {
            ...
            if (!_nested)
            {
                c = CommitInternal(serviceHeader);
            }
        ...
        public Task<int> SaveChangesAsync(ServiceHeader serviceHeader)
        {
            return SaveChangesAsync(serviceHeader, CancellationToken.None);
        }

        public async Task<int> SaveChangesAsync(ServiceHeader serviceHeader, CancellationToken cancelToken)
        {
            if (cancelToken == null)
            ...
            if (!_nested)
            {
                c = await CommitInternalAsync(serviceHeader, cancelToken).ConfigureAwait(false);
            }
        ...
        private int CommitInternal(ServiceHeader serviceHeader)
        {
            return _dbContexts.Commit(serviceHeader);
        }

        private Task<int> CommitInternalAsync(ServiceHeader serviceHeader, CancellationToken cancelToken)
        {
            return _dbContexts.CommitAsync(serviceHeader, cancelToken);
        }
