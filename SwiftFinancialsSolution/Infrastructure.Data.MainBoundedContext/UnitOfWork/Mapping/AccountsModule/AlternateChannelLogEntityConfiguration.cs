...
{
    class AlternateChannelLogEntityConfiguration : EntityTypeConfiguration<AlternateChannelLog>
    {
        public AlternateChannelLogEntityConfiguration()
        {
            HasKey(x => x.Id);
            ...
            Property(t => t.CreatedDate).HasColumnAnnotation(IndexAnnotation.AnnotationName, new IndexAnnotation(new IndexAttribute("IX_AlternateChannelLog_CreatedDate")));
           
            ToTable(string.Format("{0}AlternateChannelLogs", DefaultSettings.Instance.TablePrefix));
        }
    }
}
