...
namespace DistributedServices.Seedwork.ErrorHandlers
{
    ...
    public sealed class ApplicationErrorHandler : IErrorHandler
    {
        ...
        /// <returns> true if should not abort the session (if there is one) and instance context if the instance context is not System.ServiceModel.InstanceContextMode.Single; otherwise, false. The default is false. </returns>
        ...
        public void ProvideFault(Exception error, MessageVersion version, ref Message fault)
        {
            ...
            else
            {
                ...
                {
                    ErrorMessage = string.Format("{0}", error.Message) ?? Messages.message_DefaultErrorMessage
                };

                //Create fault exception and message fault
                var defaultFaultException = new FaultException<ApplicationServiceError>(defaultError, new FaultReason(error.Message));
