...
namespace TestApis.Controllers
{
    ...
    public class CustomerAccountsController : ApiController
    {
        ...
        public IHttpActionResult CreateAccountForCustomer([FromBody] CreateAccountRequest request)
        {
            try
            {
                ...
                if (request.ProductCode <= 0)
                    return Content(System.Net.HttpStatusCode.BadRequest,
                                   new { success = false, message = "ProductCode is required." });

                var account = _service.CreateAccountForCustomer(
                    request.CustomerId, request.ProductCode, request.BranchId);

                return ApiResponse(true, "Account created successfully.", account);
            }
        ...
        public class CreateAccountRequest
        {
            ...
            public int ProductCode { get; set; }   // e.g. 17 for M-WALLET, 1 for Deposits, 2 for Share Capital
